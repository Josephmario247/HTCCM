...
const API_URL = window.__ENV__?.API_URL || VITE_API_URL
...
const PersonalInfo = ({ formData, handleChange }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FormInput label="Title" name="title" type="select" onChange={handleChange} options={['Mr', 'Mrs', 'Miss', 'Dr', 'Engr', 'Prof']} />
...
const Add2 = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/member/add`, formDataObj, {
                headers: {
    ...
    const highestRegNO = async () => {
        ...
        try {
            const response = await axios.get(`${API_URL}/api/highestRegNO/regNo`, {
                headers: {
                  'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
              })
    ...
    return (
        <div className='max-w-3xl mx-auto bg-white p-8 rounded-md shadow-md mt-10 md:mt-20 mb-10'>
            ...
            <div className='flex justify-end items-center m-2'>
                ...
                <div className='rounded-md bg-green-100 border text-xl font-bold w-auto h-auto p-2 items-center hover:shadow-md text-black'> Reg_No: <span className=' text-[#e93df5] font-bold cursor-pointer '>{nextRegNo}</span></div>
            </div>
